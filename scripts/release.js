...
const expectedGithubRepoSlug = "bestimmaa/matrix-workouts-core";
...
function isExpectedGithubRemote(url) {
  // Derived from the slug above rather than repeated: two copies of the repo name is
  // one copy too many for a script whose job is to refuse to publish to the wrong place.
  return new RegExp(`github\\.com[:/]${expectedGithubRepoSlug}(?:\\.git)?$`, "i").test(url);
}
